...
# molecular got this from http://pypi.python.org/pypi/websocket-client/0.4
...
class WebSocket(object):
    ...
    def recv(self):
        ...
        """
        b = self._recv(1)
        #if enableTrace: logger.debug("recv frame: " + repr(b))

        # mtgox sends "", ord(b) wants char = str of len 1
        frame_type = None
        if b: frame_type = ord(b)
        else: eReport(logger, "recv: empty string")
    ...
    def _recv(self, bufsize):
        ...
        if not bytes:
            #raise ConnectionClosedException(bytes)
            pass
        return bytes
